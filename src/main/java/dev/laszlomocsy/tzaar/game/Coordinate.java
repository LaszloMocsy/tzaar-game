...
public record Coordinate(int x, int y) {
    ...
    private static boolean hasSameXAxis(Coordinate a, Coordinate b) {
        if (a.x == 5 && b.x == 5) {
    ...
    private static boolean hasSameYAxis(Coordinate a, Coordinate b) {
        if (a.y == 5 && b.y == 5) {
    ...
    private static boolean hasSameZAxis(Coordinate a, Coordinate b) {
    ...
    public static boolean isOnSameAxis(Coordinate a, Coordinate b) {
        return hasSameXAxis(a, b) || hasSameYAxis(a, b) || hasSameZAxis(a, b);
    }

    /// Returns all coordinates between two coordinates' same axis.
    public static List<Coordinate> coordinatesBetween(Coordinate a, Coordinate b) {
        ...
        if (hasSameXAxis(a, b)) {
            ...
            for (int i = 1; i <= diff; i++) {
                list.add(new Coordinate(a.x, a.y + direction * i));
            }
        } else {
            // Y and Z axis
